...
* **Routine safety**. You can run multiple parsers in different goroutines without ill effect. There is no dependence on global shared state.
...
* **Standards compliant**. Output successfully validates using the W3C validation tool for HTML 4.01 and XHTML 1.0 Transitional. [this is a link](https://github.com/kataras/iris) `)
...
func registerCacheRoute(app *iris.Application) {
	app.Get("/markdown", cache.Handler(10*time.Second), writeMarkdown)
